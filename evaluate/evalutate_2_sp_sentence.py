...
def number2sentence(number, data):
    ...
    for index, j in enumerate(context):
        ...
        if title in support_dic:
            document_sentence_number = 0
            for sent in j[1]:
                if str(total_sentence_number) in number:
                    sentence_set.append(sent)

                if document_sentence_number in support_dic[title]:
                    gold_set.append(j[1][document_sentence_number])
    ...
    return gold_set, sentence_set


with open("output/experiment2_few_shot/2-shot/output_clean.json", "r", encoding="UTF8") as f:
    json_data = json.load(f)


with open("data/new_hotpot.json", "r", encoding="UTF8") as f:
    real_data = json.load(f)

total_f1 = 0.0
total_recall = 0.0
total_precision = 0.0
with open("output/experiment2_few_shot/2-shot/output_sentence_f1.txt", "w", encoding="UTF-8") as out_file:
    for i, data in enumerate(json_data):
        data_number = data["data_number"]
        real_answer = data["real_answer"]
        gpt_answer = data["gpt_answer"]
        real_support = data["real_support"]  # 번호로 나온걸 문장으로 변경해야함
        gpt_support = data["gpt_support"]

        # 번호를 문장으로 변경하는 함수만들기

        gold_sentence, pred_sentence = number2sentence(gpt_support, real_data[i])

        em, f1, recall, precision = get_metrics(pred_sentence, gold_sentence)
